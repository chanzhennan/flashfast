...
__global__ __forceinline__ void dual_fast_gemv_acc_fp16_silu_dot_kernel(
  ...
  T2 temp_sum_2 = {convert_fp32_to_fp16<T>(0.0f), convert_fp32_to_fp16<T>(0.0f)};
...
#pragma unroll
  ...
  float sum_2 = convert_fp16_to_fp32(__hadd(temp_sum_2.x, temp_sum_2.y));
  ...
  if (tid == 0) {
    ...
    res[row] = convert_fp32_to_fp16<T>(sum_1 * sum_2);
  }
}

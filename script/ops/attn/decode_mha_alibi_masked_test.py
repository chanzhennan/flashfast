...
from flashfast import decode_mha_alibi_masked_ut as decode_mha_alibi_masked
import argparse
...
def pytorch_attn(q, k, v, scale, alibi_mask):
    q = q.transpose(1, 2)
    k = k.transpose(1, 2)
    v = v.transpose(1, 2)
    p = torch.matmul(q, k.transpose(2, 3)) * scale
    p = p.float() + alibi_mask
...
for BS,LEN,HN,HS in Configs:
    ...
    slopes = torch.rand((HN), dtype=torch.float32, device="cuda")
    alibi_mask = slopes.unsqueeze(1) * torch.arange(LEN, device=slopes.device)
    alibi_mask = alibi_mask.unsqueeze(0).unsqueeze(2).expand(-1, -1, QLEN, -1)
    if QLEN > 1:
    ...
    ref_out = pytorch_attn(q.reshape((BS, QLEN, HN, HS)), k[:BS, :LEN], v[:BS, :LEN], scale, alibi_mask)
    all_close_list = []
    max_bias_list = []
    
    for strategy in Strategies:
        infini_out = decode_mha_alibi_masked(q, k[:BS,:LEN], v[:BS,:LEN], slopes, scale, 8.0, LEN, strategy)
    ...
    if not BENCHMARK_FLAG:
        ...
        ref_dur = benchmark(pytorch_attn, (q.reshape((BS, QLEN, HN, HS)), k[:BS, :LEN], v[:BS, :LEN], scale, alibi_mask))
        infini_dur_list = []
        for strategy in Strategies:
            infini_dur = benchmark(decode_mha_alibi_masked, (q, k[:BS,:LEN], v[:BS,:LEN], slopes, scale, 8.0, LEN, strategy)) 

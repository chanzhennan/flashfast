...
from flashfast import decode_mqa_t_ut as decode_mqa
import argparse
...
def pytorch_attn(q, k, v, scale, mask=None):
    # k.shape = (LEN, BS, HN_KV, HS), reshape to (LEN, BS, HN_KV, 1, HS), then expand to (LEN, BS, HN_KV, HN // HN_KV, HS), finally reshape to (LEN, BS, HN, HS)
    k = k.unsqueeze(-2).expand(-1, -1, -1, HN // HN_KV, -1).contiguous().view(LEN, BS, HN, HS)
    v = v.unsqueeze(-2).expand(-1, -1, -1, HN // HN_KV, -1).contiguous().view(LEN, BS, HN, HS)
    q = q.transpose(0, 2)
    k = k.transpose(0, 2)
    v = v.transpose(0, 2)
...
for BS,LEN,HN,HN_KV,HS in Configs:
    ...
    q = torch.empty((QLEN, BS, HN, HS), dtype=torch.float16, device="cuda").normal_(mean=0., std=0.5)
    k = torch.empty((MAX_SEQ_LEN, MAX_BS, HN_KV, HS), dtype=torch.float16, device="cuda").normal_(mean=0., std=0.5)
    v = torch.empty((MAX_SEQ_LEN, MAX_BS, HN_KV, HS), dtype=torch.float16, device="cuda").normal_(mean=0., std=0.5)
    ...
    ref_out = pytorch_attn(q.reshape((QLEN, BS, HN, HS)), k[:LEN, :BS], v[:LEN, :BS], scale, mask)
    all_close_list = []
    max_bias_list = []

    for strategy in Strategies:
        infini_out = decode_mqa(q, k[:LEN, :BS], v[:LEN, :BS], scale, 8.0, LEN, strategy)
        all_close = torch.allclose(ref_out, infini_out.reshape((QLEN, BS, HN, HS)), atol=1e-2, rtol=1e-3)
        max_bias = (abs(ref_out - infini_out.reshape((QLEN, BS, HN, HS)))).max()
    ...
    if not BENCHMARK_FLAG:
        ...
        ref_dur = benchmark(pytorch_attn, (q.reshape((QLEN, BS, HN, HS)), k[:LEN, :BS], v[:LEN, :BS], scale, mask))
        infini_dur_list = []
        for strategy in Strategies:
            infini_dur = benchmark(decode_mqa, (q, k[:LEN, :BS], v[:LEN, :BS], scale, 8.0, LEN, strategy)) 
